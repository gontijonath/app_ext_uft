 ...
 *
 * Cada campo é estado do Compose (`by mutableStateOf(...)`), não uma
 * constante fixa: o texto abaixo é o padrão embutido no app, mas
 * [carregarConteudoRemoto] pode substituí-lo pelo conteúdo mais atual do
 * Supabase assim que o app abre com internet. Se não houver internet, ou o
 * Supabase estiver fora do ar, o app simplesmente fica com o texto daqui —
 * nunca quebra, nunca mostra tela em branco. Nenhuma tela em `ui/telas`
 * precisa saber dessa diferença: todas continuam só lendo `Conteudo.X`.
 */
object Conteudo {

    var NOME_PLATAFORMA by mutableStateOf("Ester Sabino")
    var NOME_GRUPO by mutableStateOf("Grupo Ester Sabino")
    var SUBTITULO_PLATAFORMA by mutableStateOf("Ações de Extensão da UFT")
    var UNIVERSIDADE by mutableStateOf("Universidade Federal do Tocantins")

    // ---------------------------------------------------------------- Início

    var INICIO_SAUDACAO by mutableStateOf("Bem-vindo à plataforma")
    var INICIO_APRESENTACAO by mutableStateOf(
    ...
    var INICIO_CHAMADA_PROJETOS by mutableStateOf("Conheça os projetos")

    // -------------------------------------------------------------- Projetos

    var projetos: List<Projeto> by mutableStateOf(
        listOf(
            Projeto(
                id = "projeto-1",
                titulo = "CESAU",
    ...
    var evento: Evento by mutableStateOf(
        Evento(
    ...
    var SAIBA_TITULO_NOME by mutableStateOf("Por que \"Ester Sabino\"?")
    var SAIBA_TEXTO_NOME by mutableStateOf(
    ...
    var SAIBA_TITULO_QUEM by mutableStateOf("Quem é Ester Sabino")
    var SAIBA_TEXTO_QUEM by mutableStateOf(
    ...
    var saibaFeitos: List<String> by mutableStateOf(
        listOf(
    ...
    var SAIBA_TITULO_LIGACAO by mutableStateOf("A ligação com a extensão")
    var SAIBA_TEXTO_LIGACAO by mutableStateOf(
    ...
    var SAIBA_AVISO_DADOS by mutableStateOf(
        "Os dados biográficos acima devem ser conferidos com a coordenação antes da " +
            "versão final da plataforma."
    )

    var SAIBA_TITULO_CREDITOS by mutableStateOf("Créditos")
    var saibaCreditos: List<Pair<String, String>> by mutableStateOf(
        listOf(
    ...
    var SAIBA_TITULO_FONTES by mutableStateOf("Fontes consultadas")

    /** Cada projeto (pelo título) e as fontes usadas pra escrever o texto sobre ele. */
    var fontesConsultadas: List<Pair<String, List<Pair<String, String>>>> by mutableStateOf(
        listOf(
    ...
    var SAIBA_RODAPE_FONTES by mutableStateOf(
    ...
    var RODAPE_VERSAO by mutableStateOf("Versão de protótipo · conteúdo ilustrativo")
}
